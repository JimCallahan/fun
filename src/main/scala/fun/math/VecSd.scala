...
/** An immutable sparse vector of arbitrary size containing Double data.
  *
  ...
  */
class VecSd private (private val bits: BitSet, private val vs: Array[Double])
    extends TupleAccess[Double, VecSd]
    with DoubleTupleOps[VecSd]
    with VecOps[Double, VecSd]
    with Vec {
  ...
  def apply(i: Int): Double = {
    ...
    if (b == i) vs(j) else 0.0
  }

  def update(i: Int, e: Double): VecSd = {
    val iter = bits.iterator
    var j = -1
    var b = -1
    while ((b < i) && iter.hasNext) {
    ...
    if (b == i) {
      ...
      bits.toArray.indexWhere(_ > i) match {
        case -1 => new VecSd(nbits, vs :+ e)
        case n =>
          val abd = new ArrayBuilder.ofDouble
          for (i <- 0 until n) abd += vs(i)
          abd += e
          for (i <- n until vs.size) abd += vs(i)
          new VecSd(nbits, abd.result)
      }
    }
  }

  def magSq: Double = vs.map(x => x * x).reduce(_ + _)
  def mag: Double = scala.math.sqrt(magSq)
  def normalized: VecSd = {
    val m = mag
    if (Scalar.equiv(m, 0.0)) this.map(_ => 0.0)
    else (this / m)
  }

  def dot(that: VecSd): Double = {
    val at = bits.iterator
    val avt = vs.iterator
    val bt = that.bits.iterator
    val bvt = that.vs.iterator
    @tailrec def f(ai: Int, bi: Int, tl: Double): Double =
  ...
  def unary_- : VecSd = new VecSd(bits, vs.map(-_))

  def +(that: VecSd): VecSd = compwise(that)(_ + _)
  def -(that: VecSd): VecSd = compwise(that)(_ - _)
  def *(that: VecSd): VecSd = compwise(that)(_ * _)
  def /(that: VecSd): VecSd = compwise(that)(_ / _)

  def +(s: Double): VecSd = new VecSd(bits, vs.map(_ + s))
  def -(s: Double): VecSd = new VecSd(bits, vs.map(_ - s))
  def *(s: Double): VecSd = new VecSd(bits, vs.map(_ * s))
  def /(s: Double): VecSd = new VecSd(bits, vs.map(_ / s))

  def forall(p: (Double) => Boolean): Boolean = {
    val forZero = if (bits.size <= (bits.last + 1)) p(0.0) else true
    forZero && vs.forall(p)
  }

  def forall(that: VecSd)(p: (Double, Double) => Boolean): Boolean = {
    ...
    @tailrec def f(ai: Int, bi: Int): Boolean =
      if ((ai < bi) && at.hasNext && avt.hasNext)
        p(avt.next, 0.0) && f(at.next, bi)
      else if ((bi < ai) && bt.hasNext && bvt.hasNext)
        p(0.0, bvt.next) && f(ai, bt.next)
      else if (at.hasNext && avt.hasNext && bt.hasNext && bvt.hasNext)
  ...
  def equiv(that: VecSd, epsilon: Double): Boolean = forall(that)(Scalar.equiv(_, _, epsilon))
  def equiv(that: VecSd): Boolean = forall(that)(Scalar.equiv(_, _))

  def isNaN: Boolean = forany(_.isNaN)

  def forany(p: (Double) => Boolean): Boolean = forall(!p(_))
  def forany(that: VecSd)(p: (Double, Double) => Boolean): Boolean = forall(that)(!p(_, _))

  def foreach(p: (Double) => Unit): Unit = vs.foreach(p)

  def map(p: (Double) => Double): VecSd = new VecSd(bits, vs.map(p))
  ...
  def compwise(that: VecSd)(p: (Double, Double) => Double): VecSd = {
  ...
  def min(that: VecSd): VecSd = compwise(that)(_ min _)
  def max(that: VecSd): VecSd = compwise(that)(_ max _)
  def lerp(that: VecSd, t: Double): VecSd = compwise(that)(Scalar.lerp(_, _, t))
  def smoothlerp(that: VecSd, t: Double): VecSd = compwise(that)(Scalar.smoothlerp(_, _, t))

  def compwise(a: VecSd, b: VecSd)(p: (Double, Double, Double) => Double): VecSd = {
    throw new NotImplementedError
  }

  def clamp(lower: VecSd, upper: VecSd): VecSd = compwise(lower, upper)(Scalar.clamp)

  /** Convert to a String representation */
  override def toString = s"VecSd$toPretty"

  def toPretty: String =
    "(" + (bits.toArray.zip(vs).map {
      case (i, e) => "%d:%.2f".format(i, e)
    }.mkString(", ")) + ")"

  def toList: List[Double] = toArray.toList

  def toArray: Array[Double] = {
    val ary = Array.fill(bits.last + 1)(0.0)
    for ((i, j) <- bits.toArray.zipWithIndex) ary(i) = vs(j)
    ary
  }

  def toSparseList: List[(Int, Double)] = bits.toArray.zip(vs).toList
  def toSparseArray: Array[(Int, Double)] = bits.toArray.zip(vs)

  def getOrZero(i: Int): Double = if (i < size) this(i) else 0.0

  def toVecNd: VecNd = VecNd(toArray)
  def toVecSd: VecSd = this
...
object VecSd {
  def apply(vs: Array[Double]): VecSd = VecSd(vs, _ != 0)
  def apply(vs: Array[Double], keep: Double => Boolean): VecSd = {
    val pairs = vs.zipWithIndex.filter {
      case (e, _) => keep(e)
    }.map {
      case (e, i) => i -> e
    }
  ...
  def apply(seq: IndexedSeq[Double]): VecSd = VecSd(seq.toArray)

  def apply(pairs: Array[(Int, Double)]): VecSd = {
    val bits = BitSet(pairs.map(_._1): _*)
    val vs = pairs.map(_._2)
    new VecSd(bits, vs)
  }

  def apply(pairs: (Int, Double)*): VecSd = VecSd(pairs.toArray)
}
