...
class Pos2d private (val x: Double, val y: Double)
  ...
  override def toString = s"Pos2d$toPretty"
...
object Pos2d {
  ...
  def apply(p: Pos1d, y: Double): Pos2d = new Pos2d(p.x, y)

  def apply(ls: List[Double]): Pos2d =
    ls match {
      case List(x, y) => Pos2d(x, y)
      case List(x)    => Pos2d(x, 0.0)
      case _ => throw new IllegalArgumentException(
        "Construct from List requires at least 1 and no more than 2 components!")
    }

  def apply(ary: Array[Double]): Pos2d =
    ary match {
      case Array(x, y) => Pos2d(x, y)
      case Array(x)    => Pos2d(x, 0.0)
      case _ => throw new IllegalArgumentException(
        "Construct from Array requires at least 1 and no more than 2 components!")
    }

  def apply(buf: CharBuffer): Pos2d = Pos2d(buf.get.toDouble, buf.get.toDouble)
  def apply(buf: ShortBuffer): Pos2d = Pos2d(buf.get.toDouble, buf.get.toDouble)
  def apply(buf: IntBuffer): Pos2d = Pos2d(buf.get.toDouble, buf.get.toDouble)
  def apply(buf: LongBuffer): Pos2d = Pos2d(buf.get.toDouble, buf.get.toDouble)
  def apply(buf: FloatBuffer): Pos2d = Pos2d(buf.get.toDouble, buf.get.toDouble)
  def apply(buf: DoubleBuffer): Pos2d = Pos2d(buf.get, buf.get)

  def unapply(p: Pos2d): Some[(Double, Double)] = Some((p.x, p.y))

  val origin = Pos2d(0.0)
